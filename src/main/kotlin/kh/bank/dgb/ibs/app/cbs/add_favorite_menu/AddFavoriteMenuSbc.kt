package kh.bank.dgb.ibs.app.cbs.add_favorite_menu

import kh.bank.dgb.ibs.cbs.CoreBankingApiConnector
import kh.bank.dgb.ibs.common.envelope.RequestData
import kh.bank.dgb.ibs.common.envelope.ResponseData
import org.springframework.stereotype.Service

/** Port of the old `BizResultCodeType.CHANNEL_TYPE_CODE_CORP_BANKING` constant. */
private const val CHANNEL_TYPE_CODE_CORP_BANKING = "01"

@Service
class AddFavoriteMenuSbc(
	private val coreBankingApiConnector: CoreBankingApiConnector,
) {
	fun add(request: RequestData<AddFavoriteMenuRequest>): ResponseData<AddFavoriteMenuResponse> {
		val body = request.body?.copy(channelTypeCode = CHANNEL_TYPE_CODE_CORP_BANKING)
		return coreBankingApiConnector.post("CIB11300531", request.header?.languageCode, body, AddFavoriteMenuResponse::class.java)
	}
}
