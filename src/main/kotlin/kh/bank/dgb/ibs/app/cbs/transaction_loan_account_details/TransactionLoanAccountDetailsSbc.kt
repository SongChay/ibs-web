...
class TransactionLoanAccountDetailsSbc(
	...
	fun inquire(request: RequestData<TransactionLoanAccountDetailsRequest>): ResponseData<TransactionLoanAccountDetailsResponse> {
		val result = coreBankingApiConnector.post("CIB11300711", request.header?.languageCode, request.body, TransactionLoanAccountDetailsResponse::class.java)
	...
	private fun paymentStatusDescription(code: String?): String {
		return when (code) {
			"0" -> "Unpaid"
			"1" -> "Paid"
			"2" -> "Proportion"
