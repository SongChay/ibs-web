package kh.bank.dgb.ibs.configuration.filter.encryption
 ...
 *
 * Lives in its own `encryption` subpackage rather than flat in `configuration.filter` with the
 * other, unrelated filters — so the wire-level crypto boundary is visible from the package tree
 * alone, without opening any file.
 */
@Component
@Order(-200)
class EncryptedEnvelopeFilter(
	 ...
	 *
	 * Nested here (private, not `inner`) rather than a standalone file: this is the only class that
	 * constructs it, so nesting keeps that one-to-one relationship visible without a separate file.
	 */
	private class DecryptedBodyRequestWrapper(
